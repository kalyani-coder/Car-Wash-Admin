...
const Notification = require("../models/NotificationAgentModel"); // Adjust the path as needed
...
router.get("/:field/:value", async (req, res) => {
  ...
  if (value && field) {
    try {
      const bookings = await Notification.find({ [field]: value });
      res.json(bookings);
    } catch (e) {
...
router.patch("/:id", async (req, res) => {
  ...
  try {
    const updatedService = await Notification.findByIdAndUpdate(
      serviceId,
      req.body,
      {
        new: true,
